...
#SBATCH --container-mounts /shared/home/nsanka:/shared/home/nsanka,/data_lustre/user_data/nsanka/postgres:/var/lib/postgresql/data,/data_lustre/user_data/nsanka/grafana:/var/lib/grafana
...
export PGPORT={port_pg}
export GRAFANA_PORT={port_grafana}

sed -i -e "s/http_port = 3000/http_port = {port_grafana}/" /usr/share/grafana/conf/defaults.ini
/usr/share/grafana/bin/grafana server -homepath /usr/share/grafana &> grafana_log &
...
def main(partition, n_gpus, n_cpus, image):
    ...
    # Additional setup
    port_pg = random.randint(40000, 45000)
    logging.info("Postgres port %d", port_pg)
    port_grafana = random.randint(40000, 45000)
    logging.info("Grafana port %d", port_grafana)
    job_name = "grafana-server-aic"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Launch a grafana server")
    ...
    parser.add_argument(
        "--image",
        type=str,
        default="/shared/home/nsanka/Workspace/GitHub/grafana-demo/data_metrics_grafana.sqsh",
        help="Docker image",
    )
